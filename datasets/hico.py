...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, zero_shot_type):
        ...
        if zero_shot_type == 'default':
            with open('data/hico_20160224_det/indices_superclass_hico_c.json', 'r') as f:
                self.superclasses = json.load(f)
        elif zero_shot_type == 'rare_first':
            with open('data/hico_20160224_det/indices_superclass_hico_c_rf.json', 'r') as f:
                self.superclasses = json.load(f)
