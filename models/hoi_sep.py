...
import clip
from datasets.hico_text_label import hico_text_label, hico_unseen_index, hico_vb_text_label, hico_hoi_cluster6, cluster_hico6
from datasets.vcoco_text_label import vcoco_hoi_text_label, vcoco_vb_text_label, vcoco_hoi_cluster, cluster_vcoco
...
class CDNHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False, args=None):
        ...
        self.clip_visual = args.clip_visual
        if self.clip_visual:
            self.verb_embed = nn.Sequential(
    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.clip_visual:
            interaction_decoder_out = self.verb_embed(interaction_decoder_out)
        # [C(3),B,num_queries(100),num_verb_classes(29)]
        out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
               'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1], 'intd_out': interaction_decoder_out[-1], 'is_compo': False}
        if self.use_matching:
            out['pred_matching_logits'] = outputs_matching[-1]
        uctt_verb = None
        if self.uncertainty:
            uctt_verb = self.uctt(interaction_decoder_out)
            out['uctt_verb'] = uctt_verb[-1]

        if self.aux_loss:
            if self.use_matching:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, interaction_decoder_out,
                                                        outputs_matching, outputs_verb_uctt=uctt_verb)
            else:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, interaction_decoder_out, outputs_verb_uctt=uctt_verb)

        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, interaction_decoder_out,
                      outputs_matching=None, outputs_verb_uctt=None):
        ...
        if self.use_matching:
            if outputs_verb_uctt is not None:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c,
                         'pred_obj_boxes': d, 'pred_matching_logits': e, 'uctt_verb': f, 'intd_out': g, 'is_compo': False}
                        ...
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            ...
                            interaction_decoder_out[-min_dec_layers_num: -1])]
            else:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c,
                         'pred_obj_boxes': d, 'pred_matching_logits': e, 'intd_out': f, 'is_compo': False}
                        for a, b, c, d, e, f in
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            outputs_sub_coord[-min_dec_layers_num: -1], outputs_obj_coord[-min_dec_layers_num: -1],
                            outputs_matching[-min_dec_layers_num: -1], interaction_decoder_out[-min_dec_layers_num: -1])]
        else:
            if outputs_verb_uctt is not None:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                         'uctt_verb': e, 'intd_out': f, 'is_compo': False}
                        for a, b, c, d, e, f in
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            outputs_sub_coord[-min_dec_layers_num: -1], outputs_obj_coord[-min_dec_layers_num: -1],
                            outputs_verb_uctt[-min_dec_layers_num: -1], interaction_decoder_out[-min_dec_layers_num: -1])]
            else:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'intd_out': e, 'is_compo': False}
...
class CDNHOI2(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # [C(3),B,num_queries(100),num_verb_classes(29)]
        out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
               'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1], 'intd_out': obj_verb_rep[-1], 'is_compo': False}
        if self.use_matching:
            out['pred_matching_logits'] = outputs_matching[-1]
        ...
        if self.aux_loss:
            if self.use_matching:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, obj_verb_rep,
                                                        outputs_matching)
            else:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, obj_verb_rep,)
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, obj_verb_rep, outputs_matching=None):
        min_dec_layers_num = min(self.dec_layers_hopd, self.dec_layers_interaction)
        if self.use_matching:
            return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c,
                     'pred_obj_boxes': d, 'pred_matching_logits': e, 'intd_out': f, 'is_compo': False}
                    for a, b, c, d, e, f in zip(outputs_obj_class[-min_dec_layers_num : -1], outputs_verb_class[-min_dec_layers_num : -1],
                                             outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1],
                                             outputs_matching[-min_dec_layers_num : -1], obj_verb_rep[-min_dec_layers_num : -1])]
        else:
            return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'intd_out': e, 'is_compo': False}
                    for a, b, c, d, e in zip(outputs_obj_class[-min_dec_layers_num : -1], outputs_verb_class[-min_dec_layers_num : -1],
                                          outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1],
                                          obj_verb_rep[-min_dec_layers_num : -1])]
...
class CDNHOICompo(nn.Module):
    ...
    def forward(self, samples, target, matcher):
        ...
        if isinstance(samples, list):
            ...
            if self.compo_new:
                outputs_verb_class = self.verb_class_embed(interaction_decoder_out)
                if self.clip_visual:
                    obj_verb_rep = self.verb_embed(interaction_decoder_out)
            else:
                outputs_verb_class = self.verb_class_embed(obj_verb_rep)
                if self.clip_visual:
                    obj_verb_rep = self.verb_embed(interaction_decoder_out)
            ...
            if self.aux_loss:
                if self.use_matching:
                    out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                            ...
                                                            outputs_matching, obj_verb_rep, outputs_verb_uctt=uctt_verb)
                else:
                    out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                            outputs_sub_coord, outputs_obj_coord,
                                                            interaction_decoder_out=obj_verb_rep, outputs_verb_uctt=uctt_verb)
            output_without_aux = {k: v for k, v in out.items() if k != 'aux_outputs'}
            indices = matcher(output_without_aux, target)
            output_compo = self.forward_compo(human_out, obj_out, hopd_out, interaction_decoder_out, indices, memory, samples)
    ...
    def forward_compo(self, human_out, obj_out, hopd_out, interaction_decoder_out, indices, memory, samples):
        ...
        for i in range(half_bs * 2):
            ...
            if 0 not in num_HO:
                ...
                if human_out is not None:
                    cross_human_out_compo_1 = human_out[:, i, indices[i][0], :].repeat_interleave(len(indices[index][0]), dim=1)
            ...
            if num_HO[1] > 1:
                ...
                keep_idx = []
                for j in range(num_HO[1]**2):
                    if j not in remove_idx:
                        keep_idx.append(j)
                self_instance_out_compo_1 = instance_out[:, index, indices[index][0], :].repeat_interleave(len(indices[index][0]), dim=1)
                self_instance_out_compo_1 = self_instance_out_compo_1[:, keep_idx, :]
                self_interaction_decoder_out_compo_1 = interaction_decoder_out[:, index, indices[index][0], :].repeat(1, len(indices[index][0]), 1)
                self_interaction_decoder_out_compo_1 = self_interaction_decoder_out_compo_1[:, keep_idx, :]
                if human_out is not None:
                    self_human_out_compo_1 = human_out[:, index, indices[i][0], :].repeat_interleave(indices[index][0], dim=1)
                    self_human_out_compo_1 = self_human_out_compo_1[:, keep_idx, :]
        ...
        if self.compo_new:
            ...
            mask = mask.flatten(1)
            # interaction_query_embed = torch.flip(instance_out_compo[-1], dims=[0])
            ...
            memory = torch.flip(memory, dims=[1])
            mask = torch.flip(mask, dims=[0])
            pos[-1] = torch.flip(pos[-1], dims=[0])
            pos_embed = pos[-1].flatten(2).permute(2, 0, 1)
            ...
            if self.clip_visual:
                obj_verb_rep_compo = self.verb_embed(interaction_decoder_out_compo)
            # [num_decoder_layers(3),B,num_queries(100),num_verb_classes(29)]
        else:
        ...
        if self.aux_loss:
            if self.use_matching:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord,
                                                        outputs_matching, obj_verb_rep_compo, outputs_verb_uctt=uctt_verb, is_compo=True)
            else:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, interaction_decoder_out=obj_verb_rep_compo,
                                                        outputs_verb_uctt=uctt_verb, is_compo=True)
        return out
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      outputs_matching=None, interaction_decoder_out=None, outputs_verb_uctt=None, is_compo=False):
        min_dec_layers_num = min(self.dec_layers_hopd, self.dec_layers_interaction)
        if self.use_matching:
            if outputs_verb_uctt is not None:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c,
                         'pred_obj_boxes': d, 'pred_matching_logits': e, 'uctt_verb': f, 'intd_out': g, 'is_compo': is_compo}
                        for a, b, c, d, e, f, g in
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            outputs_sub_coord[-min_dec_layers_num: -1], outputs_obj_coord[-min_dec_layers_num: -1],
                            outputs_matching[-min_dec_layers_num: -1], outputs_verb_uctt[-min_dec_layers_num: -1],
                ...
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c,
                         'pred_obj_boxes': d, 'pred_matching_logits': e, 'intd_out': f, 'is_compo': is_compo}
                        for a, b, c, d, e, f in
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            outputs_sub_coord[-min_dec_layers_num: -1], outputs_obj_coord[-min_dec_layers_num: -1],
                            outputs_matching[-min_dec_layers_num: -1], interaction_decoder_out[-min_dec_layers_num: -1])]
            ...
            if outputs_verb_uctt is not None:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                         'uctt_verb': e, 'intd_out': f, 'is_compo': is_compo}
                        for a, b, c, d, e, f in
                        zip(outputs_obj_class[-min_dec_layers_num: -1], outputs_verb_class[-min_dec_layers_num: -1],
                            outputs_sub_coord[-min_dec_layers_num: -1], outputs_obj_coord[-min_dec_layers_num: -1],
                            outputs_verb_uctt[-min_dec_layers_num: -1], interaction_decoder_out[-min_dec_layers_num: -1])]
            else:
                return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'intd_out': e, 'is_compo': is_compo}
                    for a, b, c, d, e in zip(outputs_obj_class[-min_dec_layers_num : -1], outputs_verb_class[-min_dec_layers_num : -1],
                                          outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1],
                                          interaction_decoder_out[-min_dec_layers_num: -1])]
...
class SetCriterionHOI(nn.Module):
    ...
    def __init__(self, num_obj_classes, num_queries, num_verb_classes, matcher, weight_dict, eos_coef, losses, args):
        ...
        self.uncertainty = args.uncertainty
        self.compo = args.compo
        self.clip_visual = args.clip_visual
        if args.dataset_file == 'vcoco':
            self.clip_verb_v = torch.load('data/v-coco/clip_vb_visual_vcoco_nocrop.pt').to(args.device)
        if args.dataset_file == 'hico':
            self.clip_verb_v = torch.load('data/hico_20160224_det/clip_vb_visual_hico_nocrop.pt').to(args.device)
    ...
    def loss_obj_cardinality(self, outputs, targets, indices, num_interactions):
        ...
        if not pred_logits.shape[1]:
            losses = {'obj_cardinality_error': torch.tensor(0., requires_grad=True, device=device)}
            return losses
    ...
    def loss_clip_visual_vb(self, outputs, target_classes_o, indices):
        ...
        target_idx = torch.any(target_classes_o, dim=1)
        target_classes_o = target_classes_o[target_idx]
        intd_out_o = []
        ...
        for p in range(len(indices)):
            sep.append(len(indices[p][0]) + sep[p])
        for p in range(len(sep) - 1):
            intd_out_o_ = intd_out_o[sep[p]:sep[p + 1]]
            target_classes_o_ = target_classes_o[sep[p]:sep[p + 1]]
            loss_vb_embed_ = torch.tensor(0., requires_grad=True, device=outputs['intd_out'].device)
            if len(intd_out_o_):
    ...
    def forward(self, outputs, targets):
        ...
        if outputs_without_aux['pred_obj_logits'].shape[1]:
            indices = self.matcher(outputs_without_aux, targets)
        else:
            dvc = outputs_without_aux['pred_obj_logits'].device
            indices = [(torch.Tensor(0).long().to(dvc), torch.Tensor(0).long().to(dvc)),
                       (torch.Tensor(0).long().to(dvc), torch.Tensor(0).long().to(dvc))]
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                if aux_outputs['pred_obj_logits'].shape[1]:
